...
func (e *Binance) mapMarket(mar *BnbMarket) *banexg.Market {
	...
	unifiedType := ""
	if isSpot {
		unifiedType = banexg.TradeSpot
	} else if isSwap {
		unifiedType = banexg.TradeSwap
	} else if isFuture {
		unifiedType = banexg.TradeFuture
	} else if isOption {
		unifiedType = banexg.TradeOption
		isActive = false
	}
...
func makeFetchMarkets(e *Binance) banexg.FuncFetchMarkets {
	return func(params *map[string]interface{}) (banexg.MarketMap, error) {
		...
		for _, marketType := range e.CareMarkets {
			if e.Hosts.TestNet && marketType == "option" {
