...
update_help = """
@docs/help.md 这是banexg项目的索引文件，目前项目代码已经进行了很多更新，此索引文件中有不少是过时的；
请你以文件夹为单位，逐个文件夹、逐个代码文件，分析查看其包含的主要逻辑和功能，然后更新到索引文件中。
务必分批次进行，读取一些，分析一些，更新一些；
索引文件中不存在的描述进行删除，一些以实际代码逻辑为准；
保持当前索引文件风格，简要凝练，无冗余信息，概括性高。
每个文件的结构体/函数名保留不超过8个关键项，使用`等`代替大量枚举。
优先保留最具代表性和核心的结构体/函数；保持描述简洁凝练，无冗余信息。"""
